...
def T_all_to_0(links, q):
    ...
    T_prev_to_0 = np.eye(4)
    for i in range(len(Ts)):
        Ts[i] = T_prev_to_0.dot(Ts[i])
        T_prev_to_0 = np.array(Ts[i])
    return Ts
...
def T_i_to_j(links, q, i, j):
    ...
    if i > j:
        ...
        return T_inv(product(T_all_to_prev(links[j:i], q[j:i])))
    else:
        return np.eye(4)
...
class Link:
    ...
    def __init__(self, a=0, alpha=0, d=None, theta=None, mass=0, com=None, inertia=None):
